...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    const isSpeechRecognitionAPIAvailable = 'SpeechRecognition' in window
      ...
      speechRecognition.maxAlternatives = 1 //traz apenas uma alternativa
      speechRecognition.interimResults = true //traz os resultados conforme vou falando
    ...
    return (
      <Dialog.Root>
        <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 text-left p-5 gap-3 outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
          ...
          <p className="texte-sm leading-6 text-slate-400" >
          ...
          <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
            ...
            <form className="flex-1 flex flex-col">
              ...
              {isRecording ? (
                ...
                className="w-full flex items-center justify-center gap-2 bg-slate-900 py-4 texte-center text-sm text-slate-300 outline-none font-medium hover:text-slate-100"
                >
                ...
                className="w-full bg-lime-400 py-4 texte-center text-sm text-lime-950 outline-none font-medium hover:bg-lime-500"
                >
                  Salvar nota
                </button>
              )}
